...
export function NewOrganizationForm({ plans }: { plans: PlanOption[] }) {
  ...
  async function onSubmit(evt: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Organizacija je kreirana.");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <Label htmlFor="name">Naziv (interni)</Label>
        ...
        <div>
          <Label htmlFor="type">Tip organizacije</Label>
          <select
            id="type"
            name="type"
            required
        ...
        <div>
          <Label htmlFor="trialDays">Trajanje probnog perioda (dana)</Label>
          <Input
            id="trialDays"
            name="trialDays"
            type="number"
