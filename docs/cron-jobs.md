...
| `POST /api/v1/jobs/expire-subscriptions` | Daily 04:15 | Marks ended **investor** trials as `EXPIRED` and ended paid periods as `RESTRICTED`. **Skips `AGENCY`.** Heals leftover trial/RESTRICTED agencies to ACTIVE + `partner` (keeps `SUSPENDED`/`CLOSED`). |
